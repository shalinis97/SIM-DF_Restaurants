...
if st.sidebar.button("🏠 Home / Forecast"):
    st.session_state.page = "home"
...
if st.session_state.page == "home":
    if selected_dish:
        ...
        st.session_state.model = random_search.best_estimator_

        st.session_state.features = features_rf

        # Make predictions
        y_pred = st.session_state.model.predict(X_test)
        st.session_state.mae = mean_absolute_error(y_test, y_pred)
        st.session_state.r2 = r2_score(y_test, y_pred)
        st.success(f"Trained and saved new RF model for {selected_dish}. MAE: {st.session_state.mae:.2f}, R²: {st.session_state.r2:.2f}")


        st.metric(label="R² Score", value=f"{st.session_state.r2:.3f}")
        st.metric(label="MAE", value=f"{st.session_state.mae:.2f}")
    ...
    if "model" in st.session_state:
        ...
        st.metric("R²", f"{st.session_state.r2_score:.2f}")
